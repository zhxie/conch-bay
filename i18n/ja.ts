...
const ja = {
  ...
  log_in: "ログイン",
  log_in_notice:
    "ホラガイベイを使用すると、ログインせずにスケジュールの表示、リザルトのインポート、統計とトレンドの分析を行うことができます。ログインすると、リザルトとフレンドの読み込み、ギアの表示、イカリング3へのアクセスが可能になります。ログインするには、任天堂外部のnxapiに非識別情報を送信する必要があります。この情報はログする、記録または保存されません。詳しくは、プライバシーポリシーを参照してください。",
  log_in_warning:
