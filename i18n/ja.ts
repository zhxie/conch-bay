...
const ja = {
  ...
  log_in: "ログイン",
  log_in_notice:
    "ホラガイベイを使用すると、ログインせずにスケジュールとゲソタウンの表示、リザルトのインポート、統計とトレンドの分析を行うことができます。ログインすると、リザルトとフレンドの読み込み、持つギアの表示、イカリング3へのアクセスが可能になります。ログインするには、任天堂外部のiminkまたはnxapiに非識別情報を送信する必要があります。この情報はログする、記録または保存されません。詳しくは、プライバシーポリシーを参照してください。",
  log_in_warning:
  ...
  log_out_notice:
    "ログアウト後はリザルトとフレンドの読み込み、持つギアの表示、イカリング3へのアクセスができなくなります。ホラガイベイに保存されているリザルトは消去されません。",
  ...
  debug_notice:
    "デバッグ情報の漏洩により、アカウントが不正使用されたり、個人情報が漏洩したりする可能性があります。デバッグが必要な場合は、デバッグ情報を他の人と共有しないでください。",
  debug_continue: "理解しました",
